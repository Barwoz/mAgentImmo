...
function LeaveInstance()
	if instance.host then
		if #instance.players > 1 then
			ESX.ShowNotification(_Text.left_instance)
		end
...
AddEventHandler('instance:onInvite', function(_instance, type, data)
	...
	Citizen.CreateThread(function()
		while instanceInvite do
			...
			ESX.ShowHelpNotification(_Text.press_to_enter)

			if IsControlJustReleased(0, 38) then
				EnterInstance(instanceInvite)
				ESX.ShowNotification(_Text.entered_instance)
				instanceInvite = nil
			end
		end
	end)
	...
	Citizen.CreateThread(function()
		...
		if instanceInvite then
			ESX.ShowNotification(_Text.invite_expired)
			instanceInvite = nil
		end
	end)
