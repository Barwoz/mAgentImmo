...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_property:getOwnerProperties', function(OwnedProperties)
		Config.OwnedProperties = OwnedProperties
	end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_property:getOwnerProperties', function(OwnerProperties)
		Config.OwnerProperties = properties
	end)
...
RegisterNetEvent('esx_property:sendOwnerProperties')
AddEventHandler('esx_property:sendOwnerProperties', function(OwnedProperties)
	Config.OwnedProperties = OwnedProperties
end)
...
function CreateBlips()
	for i=1, #Config.Properties, 1 do
		...
		if property.entering then
			...
			AddTextComponentSubstringPlayerName(_Text.free_prop)
...
function SetPropertyOwned(name, owned)
	...
	if owned then
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentSubstringPlayerName(_Text.property)
...
function OpenGatewayMenu(property)
	if Config.EnablePlayerManagement then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway', {
			...
			align    = 'top-left',
			elements = {
				{label = _Text.owned_properties,    value = 'owned_properties'},
				{label = _Text.available_properties, value = 'available_properties'}
		}}, function(data, menu)
			...
			CurrentActionMsg  = _Text.press_to_menu
...
function OpenGatewayOwnedPropertiesMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		title    = property.name .. ' - ' .. _Text.owned_properties,
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		menu.close()
		...
		local elements = {
			{label = _Text.enter, value = 'enter'}
		}

		if not Config.EnablePlayerManagement then
			table.insert(elements, {label = _Text.leave, value = 'leave'})
		end
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		title    = property.name .. ' - ' .. _Text.available_properties,
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			title    = property.label .. ' - ' .. _Text.available_properties,
			align    = 'top-left',
			elements = {
				{label = _Text.buy, value = 'buy'},
				{label = _Text.rent, value = 'rent'},
				{label = _Text.visit, value = 'visit'}
		}}, function(data2, menu2)
			menu2.close()
...
function OpenRoomMenu(property, owner)
	local entering = nil
	local elements = {{label = _Text.invite_player,  value = 'invite_player'}}
	...
	if CurrentPropertyOwner == owner then
		table.insert(elements, {label = _Text.player_clothes, value = 'player_dressing'})
		table.insert(elements, {label = _Text.remove_cloth, value = 'remove_cloth'})
	end

	table.insert(elements, {label = _Text.remove_object,  value = 'room_inventory'})
	table.insert(elements, {label = _Text.deposit_object, value = 'player_inventory'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		if data.current.value == 'invite_player' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_invite', {
				title    = property.label .. ' - ' .. _Text.invite,
				align    = 'top-left',
				elements = elements,
			}, function(data2, menu2)
				TriggerEvent('instance:invite', 'property', GetPlayerServerId(data2.current.value), {property = property.name, owner = owner})
				ESX.ShowNotification((_Text.you_invited):format(GetPlayerName(data2.current.value)))
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		elseif data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_property:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					title    = property.label .. ' - ' .. _Text.player_clothes,
					align    = 'top-left',
					elements = elements
				}, function(data2, menu2)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('esx_property:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					title    = property.label .. ' - ' .. _Text.remove_cloth,
					align    = 'top-left',
					elements = elements
				}, function(data2, menu2)
					menu2.close()
					TriggerServerEvent('esx_property:removeOutfit', data2.current.value)
					ESX.ShowNotification(_Text.removed_cloth)
				end, function(data2, menu2)
					menu2.close()
				end)
			end)
...
function OpenRoomInventoryMenu(property, owner)
	ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
		...
		if inventory.blackMoney > 0 then
			table.insert(elements, {
				label = (_Text.dirty_money):format(inventory.blackMoney),
				type = 'item_account',
				value = 'black_money'
			})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			title    = property.label .. ' - ' .. _Text.inventory,
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			if data.current.type == 'item_weapon' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						ESX.ShowNotification(_Text.amount_invalid)
					else
						menu.close()
...
AddEventHandler('esx_property:hasEnteredMarker', function(name, part)
	...
	elseif part == 'exit' then
		...
		CurrentActionMsg  = _Text.press_to_exit
		...
		for i=1, #Config.Properties, 1 do
			...
			if property.entering and not property.disabled then
				...
				if distance < Config.DrawDistance then
					DrawMarker(Config.MarkerType, property.entering.x, property.entering.y, property.entering.z + 1, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, Config.MarkerSize.x, Config.MarkerSize.y, Config.MarkerSize.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, false, true, 2, false, nil, nil, false)
					letSleep = false
				end
			...
			if property.exit and not property.disabled then
				...
				if distance < Config.DrawDistance then
					DrawMarker(Config.MarkerType, property.exit.x, property.exit.y, property.exit.z + 1, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, Config.MarkerSize.x, Config.MarkerSize.y, Config.MarkerSize.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, false, true, 2, false, nil, nil, false)
					letSleep = false
				end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'property_menu' then
					_cPM.OpenMenuTP(CurrentActionData.property)
				...
				elseif CurrentAction == 'room_menu' then
					_cPM.OpenMenuCoffre(CurrentActionData.property, CurrentActionData.owner)
